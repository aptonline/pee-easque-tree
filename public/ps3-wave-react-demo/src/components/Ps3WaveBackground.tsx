...
export const Ps3WaveBackground: React.FC<Ps3WaveBackgroundProps> = ({
  ...
  const cssVars =
    gradient && gradient.length === 4
      ? {
          ["--ps3-bg-color-1" as string]: gradient[0],
          ["--ps3-bg-color-2" as string]: gradient[1],
          ["--ps3-bg-color-3" as string]: gradient[2],
          ["--ps3-bg-color-4" as string]: gradient[3],
        }
      : undefined;

  return (
    <div className="ps3-bg" style={cssVars}>
      <div className="ps3-bg__gradient" />
      <div className="ps3-bg__wave" ref={waveContainerRef} />
      <div className="ps3-bg__content">{children}</div>
    </div>
  );
};
